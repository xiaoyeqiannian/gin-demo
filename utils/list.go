...
// 插入元素
func SliceInsertI32(slice []int32, val int32, idx int) []int32 {
	rear := append([]int32{}, slice[idx:]...)
...
// 删除元素
func SliceDelI32(slice []int32, val int32) []int32 {
...
// 求交集
func SliceInterInt(slice1, slice2 []int) []int {
	m := make(map[int]int)
	nn := make([]int, 0)
...
func SliceInterI32(slice1, slice2 []int32) []int32 {
...
// 求并集
func SliceUnionI32(slice1, slice2 []int32) []int32 {
	m := make(map[int32]int)
	for _, v := range slice1 {
		m[v]++
	}
...
// 求补集
func SliceCompleI32(slice1, slice2 []int32) []int32 {
	m := make(map[int32]int)
	for _, v := range slice1 {
		m[v]++
	}
...
// 求差集（slice1-并集）
func SliceDiffI32(slice1, slice2 []int32) []int32 {
	m := make(map[int32]int)
	nn := make([]int32, 0)
...
// 拆分
func SliceSplitI32(slice []int32, start, end int) (src, des []int32) {
	if len(slice) == 0 {
		return
	}
	if end > len(slice) {
...
// 去重
func SliceUniqueI32(slice [][]int32) [][]int32 {
	result := make([][]int32, 0)
	for i := 0; i < len(slice); i++ {
		flag := false
...
func SliceComputeI32(slice []int32, num int) [][]int32 {
	result := [][]int32{}
	if len(slice) == num {
		...
		for i := 0; i < makeCount; i++ {
			if num > 1 {
				...
				result = append(result, []int32{slice[i]})
			}
		}
	}
...
func SliceComputeAllI32(slice []int32, num int) [][]int32 {
	result := [][]int32{}
	for i := 1; i <= num; i++ {
...
func _SlicePermutationsI32(slices [][]int32, out *[][]int32, idx int, idxes []int) {
	if idx < len(slices) {
...
func SlicePermutationsI32(slices [][]int32) [][]int32 {
	result := [][]int32{}
